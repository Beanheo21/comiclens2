...
async function searchPlanet() {
  ...
  try {
    const url = `https://corsproxy.io/?url=https://exoplanetarchive.ipac.caltech.edu/TAP/sync?query=select+pl_name,pl_rade,pl_masse,pl_eqt,pl_orbper,st_dist,disc_year,pl_bmasse+from+ps+where+default_flag=1+and+pl_name+like+'${encodeURIComponent(name).replace(/%20/g, '+')}
'&format=json`;
