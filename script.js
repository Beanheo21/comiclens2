...
\f0\fs24 \cf0 const NASA_KEY = \'93Y1Whb7D6yCivz6pcV0HXHf22D2jHrmRXJ2a78bWK\'94; // from api.nasa.gov\
\
    ...
    resultEl.innerHTML = `\
      ...
      <div class="stat-grid">\
        <div class="stat">\
          ...
          <div class="stat-value">$\{radius\}\'d7 Earth</div>\
        </div>\
        <div class="stat">\
          <div class="stat-label">Mass (vs Earth)</div>\
          <div class="stat-value">$\{mass\}\'d7 Earth</div>\
        </div>\
        <div class="stat">\
          <div class="stat-label">Avg Temperature</div>\
          <div class="stat-value">$\{temp !== null ? temp + " K (" + Math.round(temp - 273) + "\'b0C)" : "Unknown"\}</div>\
        </div>\
        <div class="stat">\
...
function getHabitability(temp, radius) \{\
  if (!temp) return \{ cls: "hab-meh", emoji: "\uc0\u55358 \u56631 ", text: "No temperature data \'97 could be paradise or a nightmare. We don't know." \};\
  const r = parseFloat(radius);\
  const inZone = temp >= 200 && temp <= 320;\
  const tooBig = r > 2.5;\
  if (inZone && !tooBig) return \{ cls: "hab-good", emoji: "\uc0\u55356 \u57137 ", text: `Temperature of $\{Math.round(temp - 273)\}\'b0C puts this in the habitable zone \'97 liquid water might exist. Radius is reasonable for a rocky world. Could be Earth-like!` \};\
  if (temp > 500) return \{ cls: "hab-bad", emoji: "\uc0\u55357 \u56613 ", text: `At $\{Math.round(temp - 273)\}\'b0C, you'd vaporize instantly. Even lead melts here. Hard pass.` \};\
  if (temp < 150) return \{ cls: "hab-bad", emoji: "\uc0\u55358 \u56778 ", text: `At $\{Math.round(temp - 273)\}\'b0C, everything is frozen solid. No liquid water, no life as we know it.` \};\
  if (tooBig) return \{ cls: "hab-meh", emoji: "\uc0\u55357 \u56488 ", text: `Temperature is okay but at $\{r\}\'d7 Earth's radius this is likely a gas giant \'97 no surface to stand on.` \};\
  return \{ cls: "hab-meh", emoji: "\uc0\u55356 \u57121 \u65039 ", text: `Marginal conditions \'97 temperature and size are borderline. Maybe survivable with the right gear.` \};\
\}\
\
...
\
  const BASE = 30; // Earth = 30px radius\
...
\
  ...
  vizEl.innerHTML = `\
    ...
    <p style="font-size:12px;color:#555;">$\{r\}\'d7 Earth's radius$\{r > 1 ? " \'97 that's " + (r*r).toFixed(1) + "\'d7 Earth's surface area" : ""\}</p>\
  `;\
\}\
\
